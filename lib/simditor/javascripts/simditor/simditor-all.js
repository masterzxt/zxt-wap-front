...
(function () {
    ...
    Uploader = (function (_super) {
        ...
        Uploader.prototype.xhrUpload = function (file) {
            ...
            if (file.params && file.params.tokenUrl) {
                //七牛图片上传
                window.qiniu.etag(file.obj).then(function (sha1) {
                    var etag = window.qiniu.etagToUrlSafeBase64(sha1);
                    formData.append("etag", etag);
                    formData.append('file', file.obj);
                    formData.append('key', etag);
                    $.ajax({
                        url: file.params.tokenUrl,
                        contentType: false,
                        type: 'POST',
                        data: {
                            etag: etag
                        ...
                        success: function (result) {
                            if (result.isExist) {
                                result.file_path = file.params.imgPath + result.key
...
(function () {
    ...
    Formatter = (function (_super) {
        ...
        Formatter.prototype._allowedTags = ['br', 'a', 'img', 'b', 'strong', 'i', 'u', 'p', 'ul', 'ol', 'li', 'blockquote', 'pre', 'h1', 'h2', 'h3', 'h4', 'hr','div','span'];

        Formatter.prototype._allowedAttributes = {
            img: ['src', 'alt', 'width', 'height', 'data-origin-src', 'data-origin-size', 'data-origin-name','class','ng-click'],
            a: ['href', 'target','class','ng-click'],
            pre: ['data-lang','class','ng-click'],
            p: ['data-indent','class','ng-click'],
            h1: ['data-indent','class','ng-click'],
            h2: ['data-indent','class','ng-click'],
            h3: ['data-indent','class','ng-click'],
            h4: ['data-indent','class','ng-click'],
            div:['class','ng-click'],
            span:['class','ng-click'],
        };
        ...
        Formatter.prototype.cleanNode = function (node, recursive) {
            ...
            if ($node.is(this._allowedTags.join(',')) || isDecoration) {
                ...
                if ($node.is('span,div, article, dl, header, footer, tr')) {
                    $node.append('<br/>');
                    contents.first().unwrap();
        ...
        Formatter.prototype.clearHtml = function (html, lineBreak) {
            ...
            container.contents().each(function (i, node) {
                ...
                if (node.nodeType === 3) {
                    ...
                    if (lineBreak && $node.is('span,br, p, div, li, tr, pre, address, artticle, aside, dl, figcaption, footer, h1, h2, h3, h4, header')) {
                        return result += '\n';
                    }
                }
            });
    ...
    HTMLButton = (function(_super) {
        ...
        HTMLButton.prototype.beautifyHTML = function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            if (html_beautify.html) {
                return html_beautify.html.apply(html_beautify, args);
            } else {
                return html_beautify.apply(null, args);
            }
        };
